...
beforeEach(async () => {
  try {
    await axios.delete(`${baseUrl}/tests/reset`);
  } catch (error) {
    console.error("Failed to reset test database:", error);
  }
});

test("Creates author, gets author, then deletes author", async () => {
  let newAuthor = {
    name: "Jane Doe",
...
test("PATCH /books/:id updates book details", async () => {
  let newAuthor = { name: "Patch Author", bio: "Loves updating books." };
  let authorResponse = await axios.post(`${baseUrl}/authors`, newAuthor);
  let authorId = authorResponse.data.id;
  ...
  let bookResponse = await axios.post(`${baseUrl}/books`, newBook);
  let bookId = bookResponse.data.id;

  let updatedBookData = { title: "Updated Title", genre: "fantasy", author_id: authorId, pub_year: "2023" };

  let patchResponse = await axios.patch(`${baseUrl}/books/${bookId}`, updatedBookData);
  expect(patchResponse.status).toBe(200);

  let { data: fetchedBook } = await axios.get(`${baseUrl}/books/${bookId}`);
  expect(fetchedBook.title).toBe("Updated Title");
  expect(fetchedBook.genre).toBe("fantasy");
});
...
test("GET /books returns books filtered by genre", async () => {
  let newAuthor = { name: "Genre Author", bio: "Writes in one genre." };
  let authorResponse = await axios.post(`${baseUrl}/authors`, newAuthor);
  let authorId = authorResponse.data.id;
...
test("GET /books returns books filtered by pub_year", async () => {
  let newAuthor = { name: "Year Author", bio: "Writes historical books." };
  let authorResponse = await axios.post(`${baseUrl}/authors`, newAuthor);
  let authorId = authorResponse.data.id;
